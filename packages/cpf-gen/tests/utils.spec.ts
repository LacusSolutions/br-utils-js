...
import { describeType, randomSequence } from '../src/utils';

describe('randomSequence', (): void => {
  interface Sequence {
    index: number;
    result: string;
  }

  function assertEach(times: number, size: number): Test<[Sequence]> {
    const results: Sequence[] = [];

    for (let i = 0; i < times; i++) {
      results.push({
        result: randomSequence(size),
        index: i + 1,
      });
    }
  ...
  describe('when generating numeric sequences', (): void => {
    assertEach(20, 32)('generated 32-length sequences (#$index)', ({ result }): void => {
      expect(result).toHaveLength(32);
    });

    assertEach(50, 100)('does not contain letters (#$index)', ({ result }): void => {
      expect(result).not.toMatch(/\D/);
    });

    assertEach(50, 100)('only contains numbers (#$index)', ({ result }): void => {
